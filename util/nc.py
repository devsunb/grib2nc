...
from grib2nc.util.string import split_path, join_path
...
class NCWriter(NCReader):
    ...
    def create_variable(self, path, data, dtype=None, fill_value=None, dims=('number_of_lines', 'pixels_per_line')):
        ...
        name = path_list[-1]
        nc = self.get_nc(path_list[:-1])
        created_var = nc.createVariable(name, dtype, dimensions=dims, fill_value=fill_value)
        if dtype is str:
            created_var[0] = data
        else:
            created_var[:] = data[:]
    ...
    def create_compound(self, path, compound):
        ...
        nc.createVariable(name, compound_type, 'compound_dim')[:] = compound.data[:]
